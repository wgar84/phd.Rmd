...
It is noteworthy that the variance among species with respect to RV values seem to indicate that RV values are sensitive to the magnitude of morphological integration, as *Papio* and *Macaca* are among the Catarrhines who display large values of this quantity [@oliveira_covariance_2009] and score higher (yellow) RV values for all local trait sets, at least when we consider interlandmark distances.
Such sensitivity might be one of the factors behind the issues we had with this metric that appeared in the simulation-based approach; interestingly enough, this effect of magnitude of integration [which is thought to emerge as a consequence of size variation; @marroig_comparison_2001; @marroig_size_2005; @porto_size_2013] seems to also affect Procrustes residuals, regardless of whether allometric variation is removed or not.
Interesting.

However, for the set of hypotheses concerning differences between Facial and Neurocranial traits (\autoref{fig:Dev}), there is substantial agreement between either tests performed using AVGi and RV values with respect to the pattern of rejection of null hypotheses.
Furthermore, for AVGi (\autoref{fig:Dev}a) values, the overall pattern of detection is similar to the pattern detected in local trait sets, for all representations; for RV values (\autoref{fig:Dev}b), there is ample support for the hypotheses that both Face and Neurocranium represent variational modules, in both local shape variables and interlandmark distances. 
These regions have marked differences in timing and pattern formation during development, as observed from the behavior of their composing units [@zelditch_ontogenetic_1989; @hallgrimsson_deciphering_2009; @lieberman_epigenetic_2011; @esteve-altava_beyond_2014]; therefore, it is expected that this more general pattern would be detected regardless of the metric we choose to represent modularity.
