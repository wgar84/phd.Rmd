...
in a classical quantitative genetics view ({\textbf{???}}), these shared
genetic effects are the result of pleiotropy and linkage disequilibrium.
Hence, a genotype/phenotype map composed of clusters of genes that
affect clusters of traits (with little overlapping among groups)
exhibits a modular organization, and this particular structure is
...
length'' or ``cranial width''. ({\textbf{???}}) introduced the notion
that such measurements should be restricted to single osteological
elements, preferably as distances between homologous features that could
be identified in a wide taxonomic coverage. ({\textbf{???}}) accomodates
this notion into the framework of morphological integration, thus
considering individual measurements over bones are local representations
of regional phenomena, that is, the functional, developmental and
genetic interactions that produce covariances among these elements.

In the past three decades, geometric morphometrics ({\textbf{???}};
Rohlf \& Slice, 1990; Bookstein, 1991) have been consolidated as a
quantitative framework for the representation of biological shape as
geometric configurations of homologous features (landmarks; Bookstein,
1991). Two principles are central to this framework: first, the
conceptual and statistical separation of size and shape as the two main
components of organismal form; second, the use of superimposition-based
methods (Generalized Procrustes Analysis; {\textbf{???}}) for the
estimation of shape statistical parameters, such as mean shape and shape
covariance matrix. This class of estimators was proposed as a solution
to the problem that landmark configurations are arbitrarily rotated and
translated in space, and such nuisance parameters are impossible to
estimate ({\textbf{???}}; Lele \& McCulloch, 2002), a situation known as
the identifiability problem ({\textbf{???}}).

Although the use of Procrustes estimators is currently widespread in
geometric morphometrics toolboxes ({\textbf{???}}; e.g. Klingenberg,
2011), it has not been without criticisms, either with respect to the
estimation of mean shape configurations (e.g. {\textbf{???}},
{\textbf{???}}; Huckemann, 2012) and of shape covariance matrices
(Walker, 2000; Linde \& Houle, 2009; Márquez \emph{et al.}, 2012).
Procrustes estimators have been shown to perform well under the
assumption of isotropic landmark covariance structure ({\textbf{???}}).
