...
Melo \& Marroig, 2015) têm demonstrado os efeitos da seleção direcional
...
\subsubsection{Modularidade}\label{modularidade}

Neste contexto, Wagner (1996) argumenta a respeito da estrutura De modo
mais geral, modularidade é uma propriedade característica que sistemas
biológicos apresentam em relação à distribuição de interações entre seus
elementos constituintes; isto é, em um dado sistema, subconjuntos de
elementos, denominados módulos, interagem entre si mais do que com
outros subconjuntos similares (Newman, 2006; Mitteroecker \& Bookstein,
2007; Wagner \emph{et al.}, 2007). Esta propriedade foi documentada em
diferentes níveis de organização biológica, por exemplo em relação à
dinâmica de redes metabólicas (e.g. Ravasz \emph{et al.}, 2002; Andrade
\emph{et al.}, 2011) e em relação à estrutura de interações entre os
indivíduos em populações (e.g. Fortuna \emph{et al.}, 2008) e entre
espécies em comunidades ecológicas (e.g. Genini \emph{et al.}, 2010). Em
relação a sistemas morfológicos, o conceito de modularidade está
...
a respeito de suas relações.

Desta forma, um mapa genótipo/fenótipo composto por grupos de genes que
afetam grupos de caracteres apresenta uma organização modular. Por
exemplo, a dissociação entre a função dos membros anteriores e
posteriores em certas linhagens de mamíferos como morcegos (Young \&
Hallgrímsson, 2005) e hominídeos (Young \emph{et al.}, 2010) está
associado à modularização do mapa genótipo/fenótipo destas estruturas,
evidenciada pela redução das correlações fenotípicas entre membros
anteriores e posteriores e um aumento das correlações entre elementos
constituintes de ambos os membros.
...
protecting both brain and eye, and with the overall bearing of the skull
and its connections with the remaining organismal systems; the Vault
also houses muscle attachment sites associated with mandibular
articulation, indicating that, to some degree, regions may be involved
in more than one function. The contrast between these regions is the
result of distinct gene expression profiles, which are further changed
by the diffusion of signaling factors, thus generating a feedback loop
of cell and tissue differentiation (Turing, 1952; Marcucio \emph{et
al.}, 2005; Meinhardt, 2008; Franz-Odendaal, 2011; Xu \emph{et al.},
2015).
...
from a probable insectivore ancestor towards more specialized diets,
such as frugivory or sanguivory, imply distinct functional relationships
among mandibular components; thus, such reorganization may indicate
...
(1999); Cheverud \& Marroig (2007);{]}, and such methods are often
...
limitation (e.g. Hansen \& Houle, 2008; Hine \emph{et al.}, 2009;
Marroig \emph{et al.}, 2011), the second issue is usually resolved by
comparing the set of pairwise matrix comparisons with the set of
phylogenetic distances among lineages (e.g. Marroig \& Cheverud, 2001;
Oliveira \emph{et al.}, 2009b). Although such comparison provides a
first approximation to this problem, it has the same problems as
comparing covariance matrices themselves, that is, summarising the
...
distribution (\autoref{fig:eval}). Considering the subtle differences in
$\mathbf{P}$-matrix diversity such divergence represents, and that the
ancestral population for all Platyrrhines arrived at South America
traversing the young Atlantic Ocean through the colonization of
successive island environments that may have occurred between Africa and
South America during the Miocene (Oliveira \emph{et al.}, 2009a), the
most simple explanation for such divergence in $\mathbf{P}$-matrix
structure lies in changes in the genetic architecture of cranial traits
due to founder effects (Goodnight, 2000), as the effective population
sizes in the ancestral population for all Platyrrhines probably
plummeted during such transition.
...
distribution over macroevolutionay timescales may indicate that the
fitness distribution itself may be stable as well.
