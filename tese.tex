...
contexto, tal estrutura de interações pleiotrópicas emerge devido à
seleção para tarefas conflitantes (Wagner \& Altenberg, 1996;
...
expressas em $\mathbf{G}$ representam a co-dependência entre caracteres
...
caracteres, visto que $\mathbf{G}$ deve refletir os padrões impostos por
...
linear da paisagem adaptativa também pode impactar as covariâncias
...
pelo suporte do crânio e por suas conexões com os sistemas organismais
restantes; a Abóbada também abriga locais de ancoramento da musculatura
associada à articulação da mandíbula, o que indica que, em algum grau,
cada região pode estar envolvida em mais do que uma função e que algumas
...
entendimento da dinâmica ontogénetica/funcional subjacente às
características morfológicas (Polly, 2008; Zelditch \& Swiderski, 2011).
O desenvolvimento do crânio em mamíferos ocorre através de uma série de
passos, tais como o crescimento do Neurocrânio induzido pelo crescimento
do cérebro e do crescimento mediado por interações entre músculos e
ossos, com uma relativa sobreposição espaço-temporal (Hallgrímsson \&
Lieberman, 2008; Herring, 2011; Cardini \& Polly, 2013). Estes processos
específicos resultam de padrões distintos de expressão gênica, os quais
são subsequentemente alterados pela difusão de fatores de transcrição,
gerando assim um processo de retroalimentação que produz diferenciação
entre células e tecidos (Turing, 1952; Marcucio \emph{et al.}, 2005;
Meinhardt, 2008; Franz-Odendaal, 2011; Xu \emph{et al.}, 2015). Esses
fatores de transcrição afetam linhagens de células específicas, de forma
que cada processo pode ser entendido como modular; no entanto, o contato
entre tecidos adjacentes podem produzir alterações correlacionadas
devido a interacções mecânicas e também através de cascatas de
sinalização mútuas que induzem comportamentos específicos em tecidos
vizinhos, e estas interações são necessárias para o desenvolvimento
correto de ambos tecidos (Cheverud \emph{et al.}, 1992; Ravosa \emph{et
al.}, 2000; Jiang \emph{et al.}, 2002; Marcucio \emph{et al.}, 2011).
...
pela atividade muscular relacionada com a mastigação; este efeito domina
o crescimento pós-natal do crânio (Zelditch \emph{et al.}, 1992;
Herring, 2011), visto que a atividade muscular afeta principalmente a
região Oral em que são exercidas forças musculares, mas outras regiões
do crânio em que estes músculos estão ancorados, como o Zigomático e a
Abóbada, também são afetadas em menor grau. Portanto, o desenvolvimento
é composto de uma série destes eventos, e pode ser difícil isolar o
efeito de cada processo individual tem sobre a estrutura de covariância,
dada a sua sobreposição espacial (Hallgrímsson \emph{et al.}, 2009);
assim, tanto a hierarquia temporal quanto a organização espacial das
interações ontogenéticas e funcionais podem influenciar os padrões
...
em foco afeta uma série de processos de desenvolvimento dentro de uma
única linhagem de células (\emph{efeito genético intrínsico}), e outra
em que processos em uma linhagem de células dependem da expressão do
\emph{locus} em foco em outra linhagem (\emph{efeito epigenético
extrínsico}); portanto, tais definições conferem estrutura espacial para
as interações pleiotrópicas e epistáticas.

A variação das taxas de crescimento pós-natal implica na emergência da
variação de tamanho (Pélabon \emph{et al.}, 2013; Porto \emph{et al.},
2013) e tal fonte de variação tem uma importância particular no contexto
dos sistemas morfológicos em mamíferos, afetando o nível geral de
correlações entre caracteres morfológicos (Wagner, 1984; Young \&
Hallgrímsson, 2005). Esta magnitude de integração tem consequências
...
como estas linhagens interagem com seus ambientes (Gould, 1966; West
\emph{et al.}, 1997).

Relações de escala distintas entre caracteres organísmicos e tamanho do
corpo são denominadas alometria (Huxley, 1932; Gould, 1966; Cheverud,
1982b; Lande, 1985) e podem ser representadas como leis de potência, que
se tornam lineares em escala logarítmica (Huxley, 1932; mas ver Nijhout
\& German, 2012). No contexto delimitado até o momento, alometria pode
ser entendida como um tipo particular de relação entre caracteres entre
duas escalas distintas de organização, visto que tamanho é um
propriedade emergente do organismo como um todo, enquanto o impacto
diferencial desta propriedade sobre diferentes partes do organismo
produzem aquilo que reconhecemos como alometria (Young \& Hallgrímsson,
2005), de modo que a variação de tamanho em conjunto com as relações
alométricas é o componente que abriga grande parte da variação
morfológica, especialmente em mamíferos (Wagner, 1984; Marroig \&
Cheverud, 2001).
...
as linhagens do grupo irmão Catarrhini, o mesmo cenário poderia explicar
a diversidade morfológica no crânio em Cercopithecidae (Cardini \emph{et
al.}, 2007; Cardini \& Elton, 2008). Em relação aos hominídeos, seleção
direcional para a redução de elementos da Face e aumento da capacidade
do Neurocrânio desempenhou um papel limitado na divergência entre
\emph{Homo} e \emph{Australopithecus} (Ackermann \& Cheverud, 2004;
Schroeder \emph{et al.}, 2014). No entanto, relações alométricas são
dependentes da taxa de crescimento médio de indivíduos em uma população
e também da covariância entre estas taxas de crescimento e o tamanho
corporal atingido na idade adulta; assim, seleção sobre o tamanho
corporal também pode mudar estas relações (Pélabon \emph{et al.}, 2013;
Voje \emph{et al.}, 2013). Evidências experimentais sobre a evolução das
relações alométricas (e.g. Frankino \emph{et al.}, 2005; Tobler \&
Nijhout, 2010; Egset \emph{et al.}, 2012; Bolstad \emph{et al.}, 2015)
indicam que estes parâmetros de crescimento abrigam variação genética e
estão portanto sujeito aos efeitos da seleção natural; mudanças nestes
parâmetros de crescimento então levam à mudanças nas relações
alométricas na população.

A evidência empírica sobre as mudanças a longo prazo na estrutura de
covariância genética (genética quantitativa comparativa; Steppan
\emph{et al.}, 2002) conta com a correspondência entre $\mathbf{G}$ e
sua contraparte fenotípica $\mathbf{P}$, uma vez que estimativas de
$\mathbf{G}$ demandam tamanhos amostrais elevados e, mesmo assim, tal
estimativa é propensa a erros, visto que as unidades amostrais destas
estimativas são famílias em vez de indivíduos (Meyer, 1991; Houle \&
Meyer, 2015). A semelhança na estrutura de covariância entre
$\mathbf{P}$ e $\mathbf{G}$ (``Conjectura de Cheverud''; Cheverud, 1988;
Roff, 1995), foi suportada em diferentes sistemas de caracteres e
organismos (e.g.: Waitt \& Levin, 1998; Dochtermann, 2011; Garcia
\emph{et al.}, 2014), indicando que os efeitos ambientais ($\mathbf{E}$)
possivelmente apresentam uma estrutura de correlacões semelhantes a
$\mathbf{G}$, dado que estes efeitos exercem sua influência sobre
fenótipos através dos mesmos processos de desenvolvimento sobre os quais
a variação genética se estrutura (Rice, 2002, 2004).

A análise comparativa da estrutura de covariância fenotípica mostra que
$\mathbf{P}$ pode permanecer estável na escala macroevolutiva (e.g:
...
características locais da paisagem adaptativa que atuam sobre diferentes
...
\& Hallgrímsson, 2005). Um padrão similar é exibido pelos hominídeos
(Young \emph{et al.}, 2010), onde uma redução geral das correlações
entre todos os elementos é observada em relação ao padrão observado nas
demais linhagens de Anthropoidea. Outro exemplo de divergência na
estrutura de covariância entre as linhagens devido a mudanças nas
exigências funcionais é aquela observada em ruminantes (Haber, 2015),
relacionada ao aumento nas demandas metabólicas associada à ocupação de
habitats abertos, visto que a divergência de Bovidae em relação à
Cervidae está associada a uma maior integração na região Nasal. Assim, a
divergência entre linhagens irmãs podem impor uma reorganização de ambas
estruturas de covariância (genética e fenotípica) se tal divergência
está associada a relações funcionais distintas, representados pelos
componentes não-lineares de paisagem adaptativa. Portanto, estabilidade
ou divergência na estrutura de covariância em escalas macroevolutivas
são ambos cenários possíveis, e a questão de qual cenário é regra ou
exceção permanece aberta, embora algumas diferenças teóricas e
metodológicas entre diferentes autores no que diz respeito à
representação de caracteres morfológicos precisam ser consideradas de
modo a produzir uma comparação razoável entre a plausibilidade destes
cenários.
...
``comprimento do fêmur'' ou ``largura do crânio''. Pearson \& Davin
...
características homólogas que poderiam ser identificadas em linhagens
...
utilizando-se métodos de Monte Carlo (Mantel, 1967; Cheverud \emph{et
al.}, 1989).

Nas últimas três décadas, a morfometria geométrica (Bookstein, 1982,
1991; Kendall, 1984; Rohlf \& Slice, 1990; Goodall, 1991) se consolidou
...
do tamanho e da forma como componentes da variação morfológica; Em
segundo lugar, a utilização de métodos baseados em superposição (GPA:
Análise Generalizada de Procrustes; Rohlf \& Slice, 1990; Goodall, 1991)
para a estimativa de parâmetros representativos de tal variação, como
média e matriz de covariância. Estimadores de Procrustes foram propostos
como uma solução para o problema que configurações de marcos exibem em
relação às suas relações espaciais; tanto a posição e os ângulos de
rotação entre são impossíveis de estimar sem quaisquer premissas
(Goodall, 1991; Lele \& McCulloch, 2002), uma situação conhecida como o
problema de identificabilidade (Neyman \& Scott, 1948).

Embora o uso de estimadores de Procrustes é amplamente difundido na
morfometria geométrica, parte devido à distribuição de aplicativos
dedicados ao tratamento estatístico deste tipo de representação (e.g.,
Klingenberg, 2011; Adams \& Otárola-Castillo, 2013), críticas foram
elaboradas tanto em relação à estimativa de formas médias (por exemplo,
Lele, 1993; Kent \& Mardia, 1997; Huckemann, 2012) e matrizes de
covariância entre coordenadas de marcos anatômicos (Walker, 2000; Adams
...
para a forma média possuem um bom desempenho sob a premissa de estrutura
de covariância isotrópica (Kent \& Mardia, 1997), a situação em que
covariância entre coordenadas são nulas; no entanto, quando esta
premissa não se sustenta, estimadores de Procrustes para a forma média
podem sofrem impactos tanto na precisão quanto na acurácia,
especialmente quando a variação de forma é alta (Huckemann, 2011). O
exemplo fornecido por Linde \& Houle (2009) demonstra que, quando tal
suposição é quebrada, o padrão de covariâncias entre coordenadas também
são mal estimados; se a matriz de covariância entre coordenadas
desconhecida está estruturada devido às diferenças regionais em
processos responsáveis por gerar covariâncias, essa variação pode se
distribuir por toda a configuração, efetivamente apagando o sinal dos
...
2006; Linde \& Houle, 2009; Zelditch \emph{et al.}, 2009), propor
...
no presente contexto como variáveis locais de forma) referem-se a
expansões ou retrações infinitesimais em referência a uma forma média
desconhecido (Woods, 2003) estimada em locais definidos em meio aos
marcos anatômicos representados na amostra. Os autores argumentam que
tais descritores são mensurações adequadas de variação local de forma,
de modo que estes podem ser diretamente ligados aos processos biológicos
...
forma de coordenadas tridimensionais de marcos anatômicos, dos quais
...
diferentes linhagens (Fleagle, 2013). Apenas um clado particular,
Colobinae, possui especializações para uma dieta estritamente folívora,
na forma de um trato digestório similar aquele encontrado em ruminantes,
apesar de outras linhagens também preferirem este tipo de dieta; somente
dois gêneros (\emph{Homo} e \emph{Pan}) incorporam carne de outros
vertebrados em suas dietas, incluindo outros antropóides em algumas
localidades (Noë \& Bshary, 1997). À exclusão de um gênero Neotropical
(\emph{Aotus}), todas as espécies desta linhagem apresentam hábitos
diurnos; aliada à acuidade visual característica do grupo, esta
...
parte substancial dos gêneros de Catarrhini. Associa-se esse banco de
dados à hipótese filogenética ultramétrica para Anthropoidea
...
trabalhos anteriores foram reunídas em uma único base de dados, mantendo
...
\subsubsection{Descritores de Forma}\label{descritores-de-forma}
...
uma superposição de Procrustes para toda a amostra, usando o conjunto de
marcos anatômicos descritos na \autoref{tab:lms} e na
\autoref{fig:landmarks} (doravante denominados resíduos Procrustes);
distâncias euclidianas entre marcos anatômicos, descritos na
\autoref{tab:dist}; e variáveis locais de forma (Márquez \emph{et al.},
2012), que representam medidas de transformações infinitesimais de
...
matrizes que estendem o conceito de derivada à funções multivariadas, ou
seja, descrevem a taxa de variação nas diferentes direções definidas
...
Para cada unidade amostral foram obtidas 38 transformações
correspondente às localizações dos pontos médios entre os pares de
...
conjunto de dados que representa forma (isto é, desprovidos de variação
isométrica; Bookstein, 1991; Zelditch \emph{et al.}, 2004) embora
mantendo as propriedades globais do conjunto de dados de distâncias
entre marcos, como por exemplo dimensionalidade e a correspondência
destes descritores com as hipóteses de relacionamento derivadas de
interações funcionais e ontogenéticas.

Na presente tese, considerou-se apenas as propriedades associadas ao
componente simétrico de variação; portanto, antes de qualquer análise,
os efeitos de variação na assimetria foram controlados, seguindo-se o
procedimento descrito em Klingenberg et al. (2002) para estruturas
bilaterais, através da obtenção de uma configuração simétrica de marcos
anatômicos para cada indivíduo, estimando-se uma média entre cada forma
real e sua reflexão ao longo do plano sagital Para cada conjunto de
dados, também foram removidos efeitos de pouco interesse no contexto
desta tese, como dimorfismo sexual, por exemplo. Para distâncias
euclidianas entre marcos anatômicos e variáveis locais de forma estes
efeitos foram removidos através de modelos lineares multivariados
ajustados separadamente para cada espécie, de acordo com
...
multivariados sobre esse tipo de dados com o intuito de remover efeitos
fixos.
...
ordered set of phylogenetic PCs. Skull regions are indicated by the
color legend. \label{fig:loess}}
\end{figure}
