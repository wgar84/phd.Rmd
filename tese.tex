...
modularity patterns, using different Anthropoid species, with sample
units represented both as interlandmark distances and shape variables;
furthermore, we also used an approach based on the construction of
theoretical correlation matrices. In this context, these theoretical
matrices are used in order to estimate Type I and Type II error rates
for both methods. Since these methods were designed under different
frameworks, the present work also puts some effort into unifying both
methods into the same conceptual and statistical framework, in order to
produce a more meaningful comparison.
...
which correlations are sampled. For local shape variables, the
...
With respect to the relationship between significance levels and Type I
error rates, \autoref{fig:type1} shows that these quantities approach an
identity relationship very closely, even when considering different
sample sizes
...
\section{Discussão}\label{discussao-1}
