...
in \autoref{fig:cor_dist}) changes little when size variation is removed
(0.042 to 0.061); however, in a quadratic scale, this difference is much
greater for correlations with size retained (0.022) than for those with
size removed (0.004). If we consider the ratio between these values
instead of the difference, there is substantial change in both cases,
but such changes are more dramatic in the quadratic scale; for
correlations on their natural scale, the ratio between sets goes from
0.854 to 0.100 when size is removed. Squaring the average correlations,
this ratio goes from 0.730 to 0.010. Since RV is estimated by summing up
squared within-set and between-set correlations and by taking the ratio
between functions of these values, it may not be sensitive to the
difference between average correlations when size variation is removed,
although the signal of heterogeneity in correlation structure remains
roughly the same. Regarding the empirical tests, the differences in
power for tests using RV due to differences in absolute correlation
values makes the comparison between different representations difficult.
