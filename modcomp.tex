...
modularity patterns, using both real data and an approach based on the
construction of theoretical correlation matrices; in this context, these
theoretical matrices are used in order to estimate Type I and Type II
...
framework, in order to produce a more meaningful comparison.
...
null hypothesis that a hypothetical trait set is a random arrangement
without meaningful relationships. The value obtained for the real trait
set is then compared to this distribution. In the case of AVGi, we
consider that this null hypothesis is rejected when the real value is
...
correlation values for both real and permutated matrices. Significance
in both cases remain the same, and the additional step of estimating
matrix correlations would produce an unnecessary difference between the
estimation of signficance for AVGi and RV.
...
Our estimations of power for both metrics should also be controlled for
...
with both AVGi and RV metrics. We expect that power for either tests
...
removed, the Vault sub-region is also detected. With local shape
variables, more regions are detected, and the removal of allometric
variation does not change the overall pattern of detection; notably, the
Base sub-region is detected only in 3 of 42 tests. For Procrustes
residuals, the pattern of detection among sub-regions and OTUs is
diffuse; it is noteworthy that the Base sub-region is detected among
several OTUs, which contrasts this type of morphometric variable with
the remaining two types.

Tests performed using RV coefficients (\autoref{fig:RV_Func}) show a
more diffuse pattern. When interlandmark distances are considered, most
tests detect the Vault sub-region with size variation retained, and the
Base sub-region when size variation is removed. For Procrustes
residuals, few tests are able to reject their null hypothesis, detecting
only a handful of valid modular partitions. Tests performed on local
shape variables display the opposite behavior: almost all partitions are
detected, regardless of the retention or removal of allometric
variation.
...
OTU, according to the legend. \label{fig:MI_Func}}
\end{figure}
...
whether a trait set is recognized in a given OTU, according to the
legend. \label{fig:MI_Dev}}
\end{figure}
...
level regarding tests for modular structure applied on $\mathbf{C}_r$
correlation matrices. \label{fig:type1}}
\end{figure}
...
on $\mathbf{C}_s$ correlation matrices with values sampled from the
distribution of correlations between interlandmark distances.
\label{fig:type2_ed}}
\end{figure}
...
\caption{Power for both AVGi and RV metrics as a function of the chosen
significance levels with respect to tests for modular structure applied
on $\mathbf{C}_s$ matrices with values sampled from the distribution of
correlations between local shape variables. \label{fig:type2_def}}
\end{figure}
...
significance levels with respect to tests for modular structure applied
on $\mathbf{C}_s$ matrices with values sampled from the distribution of
correlations between Procrustes residuals. \label{fig:type2_sym}}
\end{figure}
