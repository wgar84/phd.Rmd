...
multiplicative nature of biological growth (Huxley, 1932). Thus, the
Oral trait set is detected as a valid variational module (upper left
panel of \autoref{fig:Func}a) since this region is strongly affected by
the induction of bone growth due to muscular activity beginning in the
...
to raw local shape variables can be observed. With respect to local
shape variables, the patterns expressed in these covariance/correlation
matrices are influenced by allometric relationships defined on a proper
scale (Jolicoeur, 1963; Houle \emph{et al.}, 2011) and local
developmental processes. Furthermore, by removing allometric effects
from local shape variables (lower right panel of \autoref{fig:Func}a),
...
growth, as Vault elements arise mostly through intramembranous
ossification, induced by the secretion of a number of signaling factors
from the outer brain membranous tissues, with a modest but necessary
contribution of mesoderm-derived tissue that undergoes endochondral
ossification (Jiang \emph{et al.}, 2002; Rice \emph{et al.}, 2003;
Franz-Odendaal, 2011; Lieberman, 2011). Because the estimation of local
shape variables excludes shape variation associated with affine
