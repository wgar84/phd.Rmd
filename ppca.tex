...
Nasal, Zygomatic, Vault) consistently exhibit higher SRD, thus
...
pPCs; each line represents a fifth order polynomial interpolation
adjusted for each skull region, according to the legend.
\label{fig:loess}}
\end{figure}
...
behavior detailed on \autoref{fig:srd_gl} for G1-3 and L1 is pervasive,
that is, Orbit and Basicranial traits along with allometric
relationships consistenly have lower average SRD scores than remaining
regions. Notice that the explained amount of variance for intermediate
pPCs (roughly from pPC 30 to 60) is very close to zero
