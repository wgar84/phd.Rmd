...
for each species, according to \autoref{fig:phylo_model}. We estimated
these models under a Bayesian framework, obtaining a posterior
distribution of $100$ covariance $P$-matrices for each OTU in our
sample.
...
pairs of matrices, according to the definition given by Mitteroecker \&
Bookstein (2009); for a given pair of positive-definite covariance
matrices $\mathbf{C}_i$ and $\mathbf{C}_j$ between $p$ traits, the
distance $d(\mathbf{C}_i, \mathbf{C}_j)$ is given by
