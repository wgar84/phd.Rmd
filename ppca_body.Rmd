...
Some particular clades are indicated (', taxa.legend(), '). \\label{fig:phylo_decdiv}')

captions $ fig.ppca_eval <- 'Characterizing $\\mathbf{P}$-matrix variation using phylogenetic Principal Components. (a) Distribution of eigenvalues associated with pPCs; positive eigenvalues are associated with phylogenetic signal; negative eigenvalues are associated with convergence. (b) Distribution of mean SRD scores for each trait with respect to pPCs; each line represents a loess interpolation adjusted for each skull region, according to the legend. \\label{fig:ppca_eval}'

captions $ fig.phylo_gl <- paste0 ('Phylogenetic structures associated with the first and last pairs of pPCs (Global 1-2 and Local 1-2, respectively), represented as the projections of each OTU over each axis.
...
Some particular clades are indicated (', taxa.legend(), '). \\label{fig:phylo_gl}')

captions $ fig.srd_gl <- 'Covariance structure variation associated with the first and last pairs of pPCs (Global 1-2 and Local 1-2, respectively), represented using posterior mean SRD scores. Traits are colored according to their assocation with each skull region, according to the legend. \\label{fig:srd_gl}'
...
plot_grid(
  ...
  nrow = 2, labels = c('a', 'b'), label_size = 24, rel_heights = c(1, 1.25), align = 'v'
  )
```
