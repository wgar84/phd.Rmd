...
Empirical evidence on changes on genetic covariance structure [comparative quantitative genetics; @steppan_comparative_2002] rely on the correspondence between patterns embedded in $\mathbf{G}$ and its phenotypic counterpart $\mathbf{P}$, since estimating $\mathbf{G}$ demands high sample sizes and nonetheless such estimate is prone to substantial error [@meyer_estimating_1991; @houle_estimating_2015].
Such correspondence between $\mathbf{P}$ and $\mathbf{G}$, denominated Cheverud's Conjecture [@cheverud_comparison_1988; @roff_estimation_1995], has been supported in many different contexts [e.g.: @waitt_genetic_1998; @dochtermann_testing_2011; @garcia_quantitative_2014], thus indicating that $\mathbf{P}$ can be used as a surrogate for $\mathbf{G}$.
Thus, the comparative analysis of covariance


  c. Development as trajectories through phenotype space

2. Functional/Developmental Interactions on the Skull
...
```{r loess, fig.width = 10, fig.height = 7, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = captions $ fig.loess}
alt.spline
```
...
b. Adaptive landscapes and ecological processes
