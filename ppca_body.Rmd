...
captions $ fig.loess <- 'Distribution of mean SRD scores for each trait with respect to pPCs; each line represents a fifth order polynomial interpolation adjusted for each skull region, according to the legend. \\label{fig:loess}'
...
The overall distribution of average posterior SRD scores along the entire set of phylogenetic PCs (\autoref{fig:loess}) indicates that this behavior detailed on \autoref{fig:srd_gl} for G1-3 and L1 is pervasive, that is, Orbit and Basicranial traits along with allometric relationships consistenly have lower average SRD scores than remaining regions. Notice that the explained amount of variance for intermediate pPCs (roughly from pPC 30 to 60) is very close to zero (\autoref{fig:eval}); thus, although matrices describing confidence intervals in these axes can be recovered, the pattern expressed by then should not be taken into account. 
