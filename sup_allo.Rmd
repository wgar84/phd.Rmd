---
title: "Supplemental Material - Allometric Constraints and Modularity"
output:
  pdf_document:
    includes:
      in_header: Tex/sup_allo_head.tex
      before_body: Tex/chap_info.tex
    fig_caption: true
    keep_tex: true
  word_document:
    fig_caption: true
    fig_width: 6
fontsize: 11pt
csl: Bib/journal-of-evolutionary-biology.csl
bibliography: Bib/tese.bib
---	

```{r options_intro, include = FALSE}
opts_chunk $ set (fig.path = 'Figures/', dev = 'pdf')
```

```{r captions_intro, include = FALSE}
captions <- list()

captions $ fig.landmarks <- 'Landmark configuration. Lines connecting landmarks indicate traits considered, either as interlandmark distances or local shape variables. Dotted and dashed lines indicate the association of each trait to *a priori* regional hypotheses of association (Face and Neurocranium, respectively). \\label{fig:landmarks}'

captions $ fig.phylo_model <- "Sample sizes and controlled fixed effects for 109 OTUs of anthropoid primates, depicted in a phylogenetic hypothesis from @springer_macroevolutionary_2012. \\label{fig:phylo_model}"

captions $ fig.cac_vs_logCS <- "The relationship between log Centroid Size and the Common Allometric Component for all anthropoid primates. \\label{fig:cac_vs_logCS}"

captions $ fig.cac_shape <- "Shape variation associated with the Common Allometric Component. \\label{fig:cac_shape}"

captions $ fig.lambda_MI <- "Distribution of Pagel's lambda for the mixed regressions between AVG Indexes for each trait set and static allometric slopes ($b_s$). \\label{fig:lambda_MI}"

captions $ fig.MI_vs_slopeW_si <- "AVG Indexes *vs.* static allometric slopes. For each local trait set, the shaded region around regression lines indicate 95\\% credible intervals around estimated regression parameters. \\label{fig:MI_vs_slopeW_si}"


```

\renewcommand{\thefigure}{S\arabic{figure}}
\renewcommand{\thetable}{S\arabic{table}}

\input{Tables/lms_en}

![`r captions $ fig.landmarks`](Figures/landmarks.png)

\input{Tables/dist_en}

```{r phylo_model, echo = FALSE, fig.width = 8.5, fig.height = 8.5, fig.cap = captions $ fig.phylo_model}
par (omi = c(0, 0.1, 0, 0.1), mar = c(0, 0, 0, 0))
plot.phylo(Tree [[1]],
           type = 'fan', cex = 0.6, label.offset = .03,
           use.edge.length = TRUE, y.lim = c(- 0.6, 0.6), x.lim = c(- 0.6, 0.6))

models.adj <- as.factor (Aux $ data.man.sp [, 2])

models.adj [30] <- 'SUB'

models.adj <- factor (as.character (models.adj),
                      levels = c('NONE', 'SUB', 'SEX', 'SUB + SEX', 'SUB * SEX'))

tiplabels(bg = brewer.pal(5, 'Set1') [as.numeric (models.adj)],
          cex = 2.2, pch = as.numeric (models.adj) + 20, col = NULL)

tiplabels(text = Aux $ sample.size, frame = 'none', cex = 0.5, col = 'white')

legend(0, 0,
       pch = 21:25,
       pt.cex = 1.3,
       cex = 0.7,
       col = 'white',
       pt.bg = brewer.pal(5, 'Set1'),
       legend = c('-', 'subsp/pop', 'sex', 'subsp/pop + sex', 'subsp/pop * sex'),
       bty = 'o', bg = 'white', xjust = 0.5, yjust = 0.5)
```

```{r cac_shape, echo = FALSE, fig.cap = captions $ fig.cac_shape, fig.width = 9.6, fig.height = 10.8}
for (i in 1:5)
  PlotShapeDeformation(allo.Data $ CAC.shapes [, , i],
                       Aux $ tessel.38, view = c(1, 2),
                       center = c(- 1, allo.Data $ cac.value [i], 0),
                       xlim = c(-2, 2), ylim = c(-2.2, 2.2), bty = 'n', scale = 2.5,
                       add = ifelse (i == 1, F, T),
                       ylab = 'Common Allometric Component',
                       graph.par = list ('mar' = c(2, 4, 0, 0)),
                       Colors = Aux $ def.hyp [-1, 'Face'] + 1,
                       palette = colorRampPalette(brewer.pal(3, 'RdBu')))

for (i in 1:5)
  PlotShapeDeformation(allo.Data $ CAC.shapes [, , i],
                       Aux $ tessel.38, view = c(1, 3),
                       center = c(1, 0, allo.Data $ cac.value [i]),
                       scale = 2.5, add = TRUE, 
                       Colors = c(Aux $ def.hyp [-1, 'Face'],
                         Aux $ def.hyp [-1, 'Face']) + 1,
                       palette = colorRampPalette(brewer.pal (3, 'RdBu')))

axis(side = 2, labels = allo.Data $ cac.value, at = allo.Data $ cac.value)

legend('bottomright', legend = c('Neuro', 'Face'), lty = 1, lwd = 3, pch = NULL,
       col = colorRampPalette(brewer.pal (3, 'RdBu'))(2), bty = 'n')
```

```{r cac_vs_logCS, echo = FALSE, fig.cap = captions $ fig.cac_vs_logCS}
allo.Plots $ CAC.vs.logCS
```

```{r cac_wac_bs, echo = FALSE, fig.cap = captions $ fig.cac_wac_bs}
ggplot (subset (allo.Data $ integra.df, pos == 'Terminal')) +
  geom_text (aes (x = W.slope, y = atanh (abs (wAC.CAC)), label = node), size = 2) +
  theme_bw() +
  xlab (expression(b[s])) + ylab('Allometric Component Correlation')
```

\input{Tables/dic_allo_im.tex}

```{r MI_vs_slopeW_si, echo = FALSE, fig.width = 6, fig.height = 8, fig.cap = captions $ fig.MI_vs_slopeW_si}
allo.Plots $ MI.vs.Wslope.si
```

```{r lambda_MI, echo = FALSE, fig.width = 5, fig.height = 5, fig.cap = captions $ fig.lambda_MI}
allo.Plots $ lambda.MI
```

# References
